...
public class LineanegocioServiceImpl extends BaseService<LineanegocioEntity> implements ILineanegocioService {
    ...
    @Override
    public ResultSearchData<LineanegocioEntity> findAllSearch(int page, int size, String sortBy, String sortOrder, int usuarioId) {
        Pageable paging = PageRequest.of(page, size, sortOrder.equals("asc") ? Sort.by(sortBy).ascending() : Sort.by(sortBy).descending());
        Page<LineanegocioEntity> pagedResult = repository.findAllByUsuarioid(paging, usuarioId);
        return (ResultSearchData<LineanegocioEntity>) this.getResultSearch(pagedResult);
    }
}
